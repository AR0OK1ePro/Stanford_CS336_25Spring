"""
Ablation Experiment 1b: Transformer with Post-Normalization
This version modifies the architecture to use post-norm instead of pre-norm.
"""
import torch
...
class transformer_block(torch.nn.Module):
    """
    A single transformer block with POST-NORMALIZATION.
    Consists of:
    - Multi-head self-attention with residual connection, then post-norm
    - SwiGLU feed-forward with residual connection, then post-norm
    """
    def __init__(self, d_model: int, num_heads: int, d_ff: int, max_seq_len: int = None, theta: float = None, device=None, dtype=None):
        """
        Initialize the transformer block with post-normalization.
    ...
    def forward(self, x: torch.Tensor, token_positions: torch.Tensor = None) -> torch.Tensor:
        """
        Forward pass for the transformer block with post-normalization.
        ...
        """
        # Apply MHA with residual connection, then post-norm
        activations = self.rmsnorm_1.forward(
            self.mha.forward(x, token_positions) + x)
        # Apply FFN with residual connection, then post-norm
        results = self.rmsnorm_2.forward(
            self.ff.forward(activations) + activations)
        return results

class transformer_lm(torch.nn.Module):
    """
    Transformer language model with POST-NORMALIZATION.
    Composed of:
    - Token embedding
    - Stack of transformer blocks (post-norm)
    - Final RMSNorm
    ...
    def __init__(self, d_model: int, 
        ...
        """
        Initialize the transformer language model with post-normalization.
    ...
    def forward(self, token_ids: torch.Tensor, token_positions: torch.Tensor = None) -> torch.Tensor:
        """
        Forward pass for the transformer language model with post-normalization.
