"""
Ablation Experiment 3: Transformer with SiLU instead of SwiGLU
This version replaces SwiGLU with a simple SiLU feed-forward network (no gating).
FFNSiLU(x) = W2 SiLU(W1x) with dff = 4 x dmodel to match parameter count.
"""
import torch
...
class SiLUFFN(torch.nn.Module):
    """
    Simple SiLU feed-forward network without gating.
    FFNSiLU(x) = W2 SiLU(W1x)
    Uses dff = 4 x dmodel to approximately match SwiGLU parameter count.
    """
    def __init__(self, d_model: int, d_ff: int, dtype=None, device=None):
        """
        Initializes the SiLU FFN layer.

        Args:
            d_model (int): Input and output feature dimension.
            d_ff (int): Inner feed-forward dimension (should be 4*d_model for parameter matching).
            dtype (torch.dtype, optional): The desired data type of the parameters.
            device (torch.device, optional): The device to store the parameters on.
        """
        super().__init__()
        ...
        # First linear projection (input to hidden)
        ...
        # Second linear projection (hidden to output)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        Applies the SiLU FFN transformation to the input tensor.
        FFNSiLU(x) = W2 SiLU(W1x)
        ...
        return einsum(self.weight_2, activated, "d_model d_ff, ... d_ff -> ... d_model")
...
class transformer_block(torch.nn.Module):
    """
    A single transformer block with SiLU FFN instead of SwiGLU.
    Consists of:
    - Pre-normed multi-head self-attention with residual connection
    - Pre-normed SiLU feed-forward with residual connection
    """
    def __init__(self, d_model: int, num_heads: int, d_ff: int, max_seq_len: int = None, theta: float = None, device=None, dtype=None):
        """
        Initialize the transformer block with SiLU FFN.

        Args:
            d_model (int): Model dimension.
            num_heads (int): Number of attention heads.
            d_ff (int): Feed-forward hidden dimension (should be 4*d_model for parameter matching).
        ...
        self.ff = SiLUFFN(d_model, d_ff, dtype=dtype, device=device)

    def forward(self, x: torch.Tensor, token_positions: torch.Tensor = None) -> torch.Tensor:
        """
        Forward pass for the transformer block with SiLU FFN.
        ...
        # Apply second RMSNorm, then SiLU FFN with residual connection
...
class transformer_lm(torch.nn.Module):
    """
    Transformer language model with SiLU FFN instead of SwiGLU.
    Composed of:
    - Token embedding
    - Stack of transformer blocks (with SiLU FFN)
    - Final RMSNorm
    ...
    def __init__(self, d_model: int, 
        ...
        """
        Initialize the transformer language model with SiLU FFN.
    ...
    def forward(self, token_ids: torch.Tensor, token_positions: torch.Tensor = None) -> torch.Tensor:
        """
        Forward pass for the transformer language model with SiLU FFN.
