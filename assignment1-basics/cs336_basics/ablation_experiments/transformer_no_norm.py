"""
Ablation Experiment 1: Transformer without Layer Normalization
This version removes RMSNorm entirely from the transformer architecture.
"""
import torch
...
class transformer_block(torch.nn.Module):
    ...
    Consists of:
    - Multi-head self-attention with residual connection (no pre-norm)
    - SwiGLU feed-forward with residual connection (no pre-norm)
    """
    def __init__(self, d_model: int, num_heads: int, d_ff: int, max_seq_len: int = None, theta: float = None, device=None, dtype=None):
        """
        Initialize the transformer block without normalization.
    ...
    def forward(self, x: torch.Tensor, token_positions: torch.Tensor = None) -> torch.Tensor:
        """
        Forward pass for the transformer block without normalization.
        ...
        """
        # Apply MHA with residual connection (no pre-norm)
        activations = self.mha.forward(x, token_positions) + x
        # Apply FFN with residual connection (no pre-norm)
        results = self.ff.forward(activations) + activations
        return results

class transformer_lm(torch.nn.Module):
    """
    Transformer language model WITHOUT layer normalization.
    Composed of:
    - Token embedding
    - Stack of transformer blocks (no normalization)
    - Output linear projection to vocabulary size (no final norm)
    """
    def __init__(self, d_model: int, 
        ...
        """
        Initialize the transformer language model without normalization.
    ...
    def forward(self, token_ids: torch.Tensor, token_positions: torch.Tensor = None) -> torch.Tensor:
        """
        Forward pass for the transformer language model without normalization.
        ...
        # Direct linear projection to vocab size (no final norm)
