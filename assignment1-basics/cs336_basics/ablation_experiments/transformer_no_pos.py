"""
Ablation Experiment 2: Transformer without Position Embedding
This version removes all positional information (RoPE) from the transformer architecture.
"""
import torch
...
class multihead_self_attention(torch.nn.Module):
    ...
    def __init__(self, d_model: int, num_heads: int, max_seq_len: int = None, theta: float = None, device=None, dtype=None):
        ...
        # No RoPE - positional encoding is completely removed
        self.rope = None
    
    def forward(self, x: torch.Tensor, token_positions: torch.Tensor = None) -> torch.Tensor:
        """
        Forward pass without any positional encoding.
        
        Args:
            x (torch.Tensor): Input tensor (..., seq_len, d_model).
            token_positions (torch.Tensor, optional): Ignored - no positional encoding.
        ...
        # No positional encoding applied to query and key
...
class transformer_block(torch.nn.Module):
    """
    A single transformer block WITHOUT positional encoding.
    Consists of:
    - Pre-normed multi-head self-attention with residual connection (no RoPE)
    ...
    def __init__(self, d_model: int, num_heads: int, d_ff: int, max_seq_len: int = None, theta: float = None, device=None, dtype=None):
        """
        Initialize the transformer block without positional encoding.

        Args:
            d_model (int): Model dimension.
            num_heads (int): Number of attention heads.
            d_ff (int): Feed-forward hidden dimension.
            max_seq_len (int, optional): Ignored - no positional encoding.
            theta (float, optional): Ignored - no positional encoding.
        ...
        # No positional encoding parameters passed
        self.mha = multihead_self_attention(d_model, num_heads, None, None, device=device, dtype=dtype)
        self.rmsnorm_2 = RMSNorm(d_model, device=device)
        self.ff = SwiGLU(d_model, d_ff, dtype=dtype, device=device)

    def forward(self, x: torch.Tensor, token_positions: torch.Tensor = None) -> torch.Tensor:
        """
        Forward pass for the transformer block without positional encoding.
        ...
        Returns:
            torch.Tensor: Output tensor (..., seq_len, d_model).
        """
        # Apply first RMSNorm, then MHA with residual connection (no positional info)
        activations = self.mha.forward(
            self.rmsnorm_1.forward(x), None) + x
...
class transformer_lm(torch.nn.Module):
    """
    Transformer language model WITHOUT positional encoding.
    Composed of:
    - Token embedding (no positional embedding)
    - Stack of transformer blocks (no RoPE)
    - Final RMSNorm
    ...
    def __init__(self, d_model: int, 
        ...
        """
        Initialize the transformer language model without positional encoding.
        ...
        Args:
            ...
            context_length (int, optional): Ignored - no positional encoding.
            theta (float, optional): Ignored - no positional encoding.
            device (torch.device, optional): Device for parameters.
            dtype (torch.dtype, optional): Data type for parameters.
        """
        ...
        # Stack of transformer blocks without positional encoding
        self.blocks = torch.nn.ModuleList([
            transformer_block(
                d_model, num_heads, d_ff, None, None, device=device, dtype=dtype 
    ...
    def forward(self, token_ids: torch.Tensor, token_positions: torch.Tensor = None) -> torch.Tensor:
        """
        Forward pass for the transformer language model without positional encoding.
        ...
        """
        # Embed input tokens (no positional embedding added)
        x = self.embedding.forward(token_ids)
        # Pass through each transformer block (no positional info)
